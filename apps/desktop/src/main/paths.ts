...
/**
 * Resolve every LDD path. `overrideDataDirectory` relocates the whole data
 * tree off the system drive: `dataRoot` becomes that directory (settings,
 * kernels, logs, cache) and `dshHome` becomes `<dir>\harness` (sessions,
 * attachments, plugins, skills). Absent, the built-in `%LOCALAPPDATA%\LDD`
 * and `%APPDATA%\LDD\harness` defaults apply.
 */
export function resolveLddPaths(
  localAppData: string,
  resourcesRoot: string,
  roamingAppData: string = localAppData,
  overrideDataDirectory?: string,
): LddPaths {
  ...
  const relocated = overrideDataDirectory !== undefined && overrideDataDirectory !== ''
  if (relocated && !isAbsolute(overrideDataDirectory)) {
    throw new TypeError('LDD data directory must be absolute')
  }
  const dataRoot = relocated ? resolve(overrideDataDirectory) : resolve(localAppData, 'LDD')
  const runtimeRoot = join(dataRoot, 'runtime')
  const normalizedResources = resolve(resourcesRoot)
  return {
    dataRoot,
    dshHome: relocated ? join(resolve(overrideDataDirectory), 'harness') : join(resolve(roamingAppData), 'LDD', 'harness'),
