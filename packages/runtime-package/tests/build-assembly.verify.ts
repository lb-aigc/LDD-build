  ...
  try {
    ...
    for (const sequence of [1, 2]) {
      ...
      assert.deepEqual(observedRuntimeNodeCommands, [
        ...
        "runtime:--eval const esbuild = await import('esbuild');const transformed = await esbuild.transform('export const ready = true');if (typeof transformed.code !== 'string' || transformed.code.length === 0) process.exit(41);await import('koffi');await import('node-pty')",
        'runtime:--help',
      ])
