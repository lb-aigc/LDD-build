...
- Image-to-image (`inputImages`): use this for ANY request that modifies an existing image — NOT just angle/viewpoint/style changes. This includes: removing an element (a person, text, watermark, an object), fixing/repairing, replacing the background, editing one part, and restyling. Pass `inputImages` as an array of the reference(s): an image URL (the URL an earlier `generate_image` returned), a local file path to a workspace image (e.g. `D:/file/inputs/ref.jpg` — pass the path AS-IS, the tool reads the file itself; NEVER write shell/Node scripts to upload it), or the sentinel `@uploaded` for an image the USER just attached. Put the edit instruction in `prompt` (e.g. "移除图中人物，保持构图与机位不变，背景自然补全"). NEVER write Python/OpenCV/PIL/compositing scripts or PowerShell/Node scripts to mask, erase, or composite images by hand — always route through `generate_image`. Midjourney (`midjourney` / `legnext`) does not support i2i; route those requests to another provider.
